...
class LongestVisitDurationPolicy {
    constructor() {
        ...
        this.minPageVisitDuration = 10000;
    }
    getItemList(menus, analyser) {
        ...
        let items = Menu_1.Menu.getAllMenusItems(menus);
...
class MostClickedItemListPolicy {
    ...
    mapItemsToNbClicks(menus, analysis) {
        let itemsMappedToNbClicks = new Map();
        let allItems = Menu_1.Menu.getAllMenusItems(menus);
    ...
    sortAnFilterMappedItems(itemsMappedToNbClicks) {
...
class MostRecentVisitsPolicy {
    ...
    getItemList(menus, analyser) {
        ...
        function findRelatedLinkNodes(node, pathname) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const Menu_1 = require("../../Menus/Menu");
class SerialPositionCurvePolicy {
    constructor() {
        // Maximum number of items to keep
        this.maxNbItems = 2;
        // If true, ignore the current page
...
class Highlight {
    ...
    static onElement(element) {
        Highlight.onNode(element.node);
    }
    static offElement(element) {
        Highlight.offNode(element.node);
    }
    static onAllElements(elements) {
        elements.forEach(this.onElement);
    }
    static offAllElements(elements) {
...
class AdaptiveElement {
    constructor(node, selector = null, parent = null) {
...
class ItemGroup extends AdaptiveElement_1.AdaptiveElement {
    ...
    static fromSelectors(selector, itemSelectors, parent) {
        ...
        for (let itemSelector of itemSelectors) {
            group.items.push(Item_1.Item.fromSelector(itemSelector, group));
        }
        return group;
    }
}
...
class Menu extends AdaptiveElement_1.AdaptiveElement {
    ...
    static fromSelectors(selector, descendantSelectors) {
        let node = $(selector);
        ...
        if (typeof descendantSelectors === "string" || Array.isArray(descendantSelectors)) {
            menu.groups.push(ItemGroup_1.ItemGroup.fromSelectors(null, descendantSelectors, menu));
        }
        else {
            for (let groupSelector in descendantSelectors) {
                let itemSelectors = descendantSelectors[groupSelector];
                menu.groups.push(ItemGroup_1.ItemGroup.fromSelectors(groupSelector, itemSelectors, menu));
            }
        }
        return menu;
    }
...
class DataAnalyser {
    ...
    analyseItemClicks() {
        ...
        function createAllFieldsIfRequired(IDs, pathname) {
        ...
        function processClick(IDs, pathname) {
        ...
        for (let itemClick of itemClickData) {
            ...
            let pathname = itemClick["pathname"];
    ...
    analysePageVisits() {
        ...
        function processPageVisit(visit) {
        ...
        function computeFrequency(nbVisits, pathname) {
...
class DataLogger {
    ...
    startListeningForItemClicks() {
        let self = this;
        let allItems = Menu_1.Menu.getAllMenusItems(this.menus);
        for (let item of allItems) {
...
class Database {
    ...
    empty(clearLocalStorage = true, defaultInit = true) {
        this.data.clear();
        if (defaultInit) {
    ...
    getTableEntries(name, test = (_) => { return true; }) {
        if (!this.data.has(name)) {
            return undefined;
        }
    ...
    addTableEntry(name, entry, checkFormat = false) {
        if (!this.data.has(name)) {
            return false;
        }
        let table = this.data.get(name);
    ...
    delayTableEntryAddition(name, entry) {
    ...
    addAllDelayedTableEntries(checkFormat = false) {
        let success = true;
    ...
    editTableEntries(name, edit, test) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const Menu_1 = require("./Menus/Menu");
const Database_1 = require("./UserData/Database");
...
const MostRecentVisitsPolicy_1 = require("./Adaptations/Policies/MostRecentVisitsPolicy");
const SerialPositionCurvePolicy_1 = require("./Adaptations/Policies/SerialPositionCurvePolicy");
class AdaptiveWebMenus {
    ...
    setAdaptation(techniqueName, policyName) {
    ...
    getAllAdaptationTechniqueNames() {
        return [...Object.keys(this.adaptations)];
    }
    getAllAdaptationPoliciesNames(techniqueName) {
    ...
    static fromMenuSelectors(selectors) {
        let menus = [];
        for (let menuSelector in selectors) {
            menus.push(Menu_1.Menu.fromSelectors(menuSelector, selectors[menuSelector]));
        }
...
$(document).ready(function () {
    ...
    let awm = awm_1.AdaptiveWebMenus.fromMenuSelectors(menuSelectors);
